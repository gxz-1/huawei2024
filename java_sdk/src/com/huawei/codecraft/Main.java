...
public class Main {
    ...
    private int[][] isCollision = new int[200][200];
    ...
    private HashSet<BFSPathSearch.Point> collisionPoint = new HashSet<>();
    ...
    private int input(Scanner scanf) {
        ...
        for (int i = 0; i < robot_num; i++) {
            ...
            if (robots[i].sts == 0) {//如果发生碰撞，需要冻结状态并等待
                collisionPoint.add(new BFSPathSearch.Point(robots[i].x, robots[i].y, 0));
                fastAStar.setBlock(robots[i].x, robots[i].y);
                ...
                if (!collisionPoint.isEmpty()) {//发生碰撞了
                    robots[i].mvPath= fastAStar.findPathCommands(robots[i].x, robots[i].y, robots[i].destinationX, robots[i].destinationY);
                }
                //从碰撞中恢复时删除障碍点
                collisionPoint.remove(new BFSPathSearch.Point(robots[i].x, robots[i].y, 0));
                fastAStar.searchArea[robots[i].x][robots[i].y].setBlock(false);
...
//                    r.searchBerth3();
    ...
    public void goFirst(int i) {
        ...
        if (hasNeighbor == 1) {//有邻居，如果在路上，那么就重新避人寻路
            if (robots[i].mvPath != null && !robots[i].mvPath.isEmpty())
                findPathBypassRobots(i, robots[i].destinationX, robots[i].destinationY);
...
//                mvPath=AStar.findPath(x, y, berth[berthId].x + 2, berth[berthId].y + 2,blockArray);
        ...
        public void searchGds1() {
            ...
            if (BestGood != null) {//才能试图寻找BestPath
                ...
                BestPath = fastAStar.findPathCommands(x, y, destinationX, destinationY);
        ...
        public void searchGds2() {
            ...
            if (BestGood != null) {//才能试图寻找BestPath
                BestPath = fastAStar.findPathCommands(x, y, BestGood.x, BestGood.y);
                if (BestPath.isEmpty() && NextBestGood != null) {//TODO 找次好的货物
                    BestPath = fastAStar.findPathCommands(x, y, NextBestGood.x, NextBestGood.y);
